        ...
        if module.params['state'] is not None:
            ...
            if 'ActiveState' in result['status']:
                ...
                if action is not DO_NOTHING:
                    result['changed'] = True
                    rc, out, err = module.run_command(r"%s %s '%s' | grep ^ExecReload=" % (systemctl, 'cat', unit))
                    if rc != 0:
                        module.warn('The service (%s) does not support `reload` action and it is likely to fail.' % unit)
                    if not module.check_mode:
                        (rc, out, err) = module.run_command("%s %s '%s'" % (systemctl, action, unit))
                        if rc != 0:
